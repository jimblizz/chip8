...
func (c* Chip8) OpNotImpl (op uint16) {
    fmt.Println(fmt.Sprintf("%X not implemeted\n", op & 0xF000))
    c.Stop()
}


func (c* Chip8) Dispatch (pc uint16, op uint16, draw *bool) error {
    fmt.Printf("%X    %X       ", pc, op)
    ...
    return nil // For now, never return an error
}

func (c* Chip8) AdvanceCycle () {
    c.PC += 2
}

func (c* Chip8) Dispatch8k (pc uint16, op uint16) {
...
func (c* Chip8) DispatchF (pc uint16, op uint16) {
